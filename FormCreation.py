...
# Build the service
    ...
    for form_url, file_name in file_data:
        row = find_row(values, file_name)
        if row == 0:
            print("matching name not found for: ", file_name)
            continue
        update_cell_with_backoff(sheet1, row, 11, form_url)
...
def get_topic(doc_id):
    ...
    if topic_num == 1:
        return file['webViewLink'], "[Coding: novel prompting methods for large language models to improve code generation]"
    if topic_num == 2:
        return file['webViewLink'], "[Math: novel prompting methods for large language models to improve mathematical problem solving]"
    if topic_num == 3:
        return file['webViewLink'], "[Factuality: novel prompting methods that can improve factuality and reduce hallucination of large language models]"
    if topic_num == 4:
        return file['webViewLink'], "[Bias: novel prompting methods to reduce social biases and stereotypes of large language models]"
    if topic_num == 5:
        return file['webViewLink'], "[Safety: novel prompting methods to improve large language models' robustness against adversarial attacks or improve their security or privacy]"
    if topic_num == 6:
        return file['webViewLink'], "[Multilingual: novel prompting methods to improve large language models’ performance on multilingual tasks or low-resource languages and vernacular languages]"
    if topic_num == 7:
        return file['webViewLink'], "[Uncertainty: novel prompting methods that can better quantify uncertainty or calibrate the confidence of large language models]"
    else:
        print("NO TOPIC FOR: ", file_name)
        return file['webViewLink'], "[No topic was returned]"
...
if not items:
    ...
    for item in items:
        ...
        doc_name = file_name[:index] + '.docx'
        doc_file_id = None
        for item2 in items2:
            if item2['name'] == doc_name:
                doc_file_id = item2['id']
                break
        
        if doc_file_id:
           
            download_link, topic = get_topic(doc_file_id)
            file_content = read_file_content_with_retry(file_id)
            index1 = file_content.find("Title:") + 7
            index2 = file_content.rfind("1. Problem Statement:")
            title = file_content[index1:index2].strip()
            form_url, form_id = create_google_form_with_retry(file_content, topic, download_link)
            file_data.append((form_url, title))
            
        else:
            download_link = drive_service.files().get(fileId=file_id, fields='webViewLink').execute()['webViewLink']
            topic = "[No topic returned]"
            print("NO CORRESPONDING DOC FOR: ", file_name)
            file_content = read_file_content_with_retry(file_id)
            index1 = file_content.find("Title:") + 7
            index2 = file_content.rfind("1. Problem Statement:")
            title = file_content[index1:index2].strip()
